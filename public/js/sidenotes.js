...
let postSelector = "div.content-main";
	...
	for (const child of postContent.children) {
		...
		if (anchors.length) {
			...
			for (anchor of anchors) {
				...
				sidenoteWrapper.insertAdjacentHTML(
					"afterbegin",
					`<div class='note-identifier'>${anchor.textContent}.</div>`
				);
...
function positionSidenotes() {
	...
	for (let i = 0; i < sidenotes.length; i++) {
		...
		const anchor = document.querySelector(
			`${postSelector} > *:not(${notesWrapperSelector}, .footnotes) #${anchorId}`
		);
...
function onAnchorClick(evt) {
	...
	dehilightNotes();
	if (mediaQuery.matches) {
		const sidenote = document.getElementById(
			evt.target.parentNode.id.replace("anchor-", "sidenote-")
		);
		if (sidenote) {
			evt.preventDefault();
			evt.stopPropagation();
